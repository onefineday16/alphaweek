 ...
 *   2. Manual: GET /api/fetch-weekly?token=<MANUAL_TRIGGER_TOKEN>
 *   3. Seed ย้อนหลัง (K3, รันครั้งเดียว): GET /api/fetch-weekly?token=<TOKEN>&seed=1
...
module.exports = async function handler(req, res) {
  // --- Auth: Vercel Cron ส่ง header x-vercel-cron / manual ต้องมี token ตรง ---
  var isCron = !!req.headers['x-vercel-cron'];
  var token = (req.query && req.query.token) || '';
  var isManual = !!process.env.MANUAL_TRIGGER_TOKEN &&
  ...
  if (seedMode && !isManual) {
    // seed ต้องเป็น manual + token เท่านั้น (กัน cron ไปรัน seed โดยไม่ตั้งใจ)
    return res.status(401).json({ ok: false, error: 'seed requires manual token' });
  }

  try {
    var result = seedMode ? await runSeed() : await runWeekly();
    return res.status(200).json({ ok: true, mode: seedMode ? 'seed' : 'weekly', result: result });
  } catch (err) {
